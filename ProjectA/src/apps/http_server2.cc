...
int main(int argc,char *argv[])
{
  ...
  /* start listening */
        listen_chk = minet_listen(sock, 1024);
        if(listen_chk < 0)
        {
  ...
  while(1)
  {
    ...
    /* do a select */
	select_chk = minet_select(maxfd+1, &read, NULL, NULL, NULL);
	if(select_chk < 0)
	{
		fprintf(stderr, "There was an error when calling select()\n");
